...
//============================================================================
//============================================================================
    function getMonodevelop() {

        var _monodevelop = monodevelop;

        function readFile(fileName, encoding) {
            if (!_monodevelop.fileExists(fileName)) {
                return undefined;
            }
            return _monodevelop.readFile(fileName)
        }
        function writeFile(fileName, data, writeByteOrderMark) {
            if (writeByteOrderMark) {
                data = '\uFEFF' + data;
            }
            _monodevelop.writeFile(fileName, data, writeByteOrderMark);
        }
        return {
            args: _monodevelop.arguments(),
            newLine: _monodevelop.newLine(),
            useCaseSensitiveFileNames: true,
            write: function (s) {
               _monodevelop.write(s);
            },
            readFile: readFile,
            writeFile: writeFile,
            watchFile: function (fileName, callback) {
                _monodevelop.watchFile(fileName, { persistent: true, interval: 250 }, fileChanged);
//                    callback(fileName);
            },
            resolvePath: function (path) {
                return _monodevelop.resolvePath(path);
            },
            fileExists: function (path) {
                return _monodevelop.fileExists(path);
            },
            directoryExists: function (path) {
                return _monodevelop.directoryExists(path);
            },
            createDirectory: function (directoryName) {
                if (!this.directoryExists(directoryName)) {
                    _monodevelop.mkdir(directoryName);
                }
            },
            getExecutingFilePath: function () {
                return _monodevelop.getExecutingFilePath();
            },
            getCurrentDirectory: function () {
                return _monodevelop.currentDirectory();
            },
            getMemoryUsage: function () {
                if (_monodevelop.garbageCollector) {
                    _monodevelop.garbageCollector();
                }
                return _monodevelop.getMemoryUsage();
            },
            exit: function (exitCode) {
                _monodevelop.exitCode(exitCode);
            }
        };
    }
//============================================================================
//============================================================================
    ...
    else if (typeof monodevelop !== "undefined") {
        return getMonodevelop();
    }
    else {
        return undefined;
    }
